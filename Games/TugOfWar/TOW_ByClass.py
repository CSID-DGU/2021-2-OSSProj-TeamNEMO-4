...
class TugOfWar:
    ...
    def game_restart(self):
        self.gam_over_timer.reset_timer()
        while True:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    return False
    ...
    def run_game_loop(self, level):
        game_over = False
        did_win = True
        nClick = self.click_n*level
        click = 0

        # 타이머 설정
        self.gam_over_timer = GameOverTimer(self.game_time)
        ...
        while not game_over:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    exit()
            left_time = self.gam_over_timer.time_checker()
            ...
            if not self.stop_timer:
                ...
                if self.click_wrong:
                    break
                message_to_screen_center(self.screen, 'CLICK A : {}'.format(timer), WHITE, level_font, self.height*(1/2))
            # 남은 클릭 수 화면에 표시
            ...
            if timer <= 0:
                ...
                message_to_screen_center(self.screen, 'CLICK D : {}'.format(time_checker), WHITE, level_font, self.height*(1/2))
                if time_checker <= 0:
                    self.stop_timer = False
                    start_ticks = pygame.time.get_ticks()
                    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000
